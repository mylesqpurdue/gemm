...
void write_csv_row(const std::string& filename, const Config& config, 
    ...
    csv << config.impl << "," << config.M << "," << config.N << "," << config.K << ","
        << config.threads << ",0,0,0," << std::fixed << std::setprecision(3) << time_ms << ","
...
int main(int argc, char** argv) {
    ...
    std::cout << "GEMM Benchmark - Milestone 1 Baseline\n";
    std::cout << "Config: M=" << config.M << ", N=" << config.N << ", K=" << config.K 
              << ", reps=" << config.reps << ", impl=" << config.impl << "\n\n";
    ...
    gemm::gemm_naive(config.M, config.N, config.K, A, B, C, lda, ldb, ldc);
    ...
    std::cout << "impl=" << config.impl << ",M=" << config.M << ",N=" << config.N 
              ...
              << ",MB=0,NB=0,KB=0,time_ms=" << std::fixed << std::setprecision(3) << best_time_ms
    ...
    if (!config.csv_path.empty()) {
        ...
        write_csv_row(config.csv_path, config, best_time_ms, gflops, final_relerr);
        std::cout << "Results written to " << config.csv_path << "\n";
    }
